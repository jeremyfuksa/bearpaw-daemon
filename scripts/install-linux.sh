#!/usr/bin/env bash
#
# Install Bearpaw as a systemd service on a Debian-family Linux host
...
#
# Usage:
#   sudo ./scripts/install-linux.sh           # install or update
#   sudo ./scripts/install-linux.sh --help    # show this help
...
preflight() {
    ...
    if ! command -v apt-get >/dev/null 2>&1; then
        die "apt-get not found; this installer assumes a Debian-family distro (Debian, Ubuntu, Raspberry Pi OS, etc.). For other distros, install the equivalent of libusb-1.0-0-dev and python3-venv manually, then \`pip install bearpaw-daemon\` into a venv and adapt packaging/systemd/bearpaw.service."
    fi
    local python_bin
...
install_wrapper() {
    ...
    cat >"${BIN_WRAPPER}" <<EOF
#!/usr/bin/env bash
# Auto-generated by bearpaw install-linux.sh
